...
(defun dmacro-get ()
  (let ((rkeys (dmacro-recent-keys)) arry)
    (if (if (featurep 'xemacs)
          ...
          (equal dmacro-keys (dmacro-subseq rkeys (- (length dmacro-keys))))) ; repeat 2回目以降は...
        (progn  ; もう見つける必要はない
          (setq *dmacro-arry-1* nil)
          *dmacro-arry*)
      ; repaet １回目だったら...
      (setq arry (dmacro-search (dmacro-subseq rkeys 0 (- (length dmacro-key)))))
      (if (null arry) ; 見つからなかったら
          (setq *dmacro-arry* nil)
        (let ((s1 (car arry)) (s2 (cdr arry))) ; XYX の繰り返しの Y X の順
...
(defun dmacro-search (array) ; XYX の繰り返しから Y X を見つける
  (let* ((arry (dmacro-array-reverse array))
    ...
    (if (null maxptr) ; maxptr が分かってないなら ここで処理 (XYX より XX を優先)
      ...
      (cons "" (dmacro-array-reverse (dmacro-subseq arry 0 maxptr))) ; maxptr が分かってれば それを採用
      )
    ))
...
(defun dmacro-array-search (pat arry &optional start) ; arry の中から pat を見つける(後ろのもの優先)
  (let* ((len (length pat))
	 (max (- (length arry) len))  ; pat でない部分の長さ
	 p found
	 )
    (setq p (if start start 0))
    (while (and (not found) (<= p max)) ; 一致する部分列が見つかる(か max に到達する)までループ
      (setq found (equal pat (dmacro-subseq arry p (+ p len))))
      (if (not found) (setq p (1+ p)))
      )
    (if found p nil) ; 見つかったら場所を返す
    ))
