...
(defun yc-string-make-unibyte (str) (if (string< "6.0"  mule-version) (string-make-unibyte str) str))
(defun yc-l2n (int)
  (yc-string-make-unibyte
...
(defun yc-unibyte-char-to-multibyte (chr) (if (string< "6.0" mule-version) (unibyte-char-to-multibyte chr) chr))
(defun yc-n2w (str)
  (let ((src
    ...
    (while src
      (setq dst (concat
		 ...
		 (decode-coding-string 
		  (concat (and (/= (car src) 0) (char-to-string (yc-unibyte-char-to-multibyte (car src))))
			  (char-to-string (yc-unibyte-char-to-multibyte (cadr src))))
		  yc-coding))
	    src (cddr src)))
    dst))
...
(defun yc-close ()
  (when yc-context
    ...
    ;; comment in 10/10/01 by knak@ceres.dti.ne.jp
    (set-process-sentinel yc-server nil)
    (condition-case nil
	(yc-finalize)
